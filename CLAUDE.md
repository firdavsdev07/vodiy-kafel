...
- **`storefront/`** — public-facing marketing/catalog site (`vodiy-kafel-storefront`), React 19 + Vite + Tailwind + GSAP + React Three Fiber. Frontend-only with local mock data, not wired to `api/`. Built independently per `api/CLAUDE.md`'s scope note ("FAQAT BACKEND — frontend alohida jamoa tomonidan yoziladi"). Has its own [storefront/README.md](storefront/README.md) and [storefront/DESIGN.md](storefront/DESIGN.md). Planned work lives in [storefront/task.txt](storefront/task.txt) — 44 tasks (`S-001`…`S-044`) in the same format as `api/task.txt`, written from a code+browser audit on 2026-09-17. Read it before changing anything here: it records why the mock data must go (the whole catalog is still `src/data/*.js`), which of its own "categories/collections/finishes" have **no backend equivalent**, and that **all 43 product images are Marazzi's copyrighted photography** — `ASSETS.md` itself says they must be replaced before any commercial launch, so that one task (`S-033`) gates going live.
- **`dashboard/`** — staff-facing admin panel consuming `api/`. The code is still just the stock Vite React template scaffold (no routing, state, or API wiring), but the work is planned: [dashboard/task.txt](dashboard/task.txt) is a dependency-ordered checklist of 48 tasks (`D-001`…`D-048`) in the same format as `api/task.txt`, derived from the real `api/openapi.json` surface. Read it before writing anything under `dashboard/` — it fixes the stack (TypeScript, Tailwind, React Router, TanStack Query), the global frontend rules, and which roles see what. Scope is **staff only** (`SUPER_ADMIN`/`MODERATOR`/`BRANCH_ADMIN`/`MANAGER`); the wholesale customer cabinet is explicitly out of scope pending an open question at the bottom of that file.
...
No tests exist yet in this package.
