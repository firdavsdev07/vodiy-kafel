...
- **`storefront/`** — public-facing marketing/catalog site (`vodiy-kafel-storefront`), React 19 + Vite + Tailwind + GSAP + React Three Fiber. Frontend-only with local mock data, not wired to `api/`. Built independently per `api/CLAUDE.md`'s scope note ("FAQAT BACKEND — frontend alohida jamoa tomonidan yoziladi"). Has its own [storefront/README.md](storefront/README.md) and [storefront/DESIGN.md](storefront/DESIGN.md).
